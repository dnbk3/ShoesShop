...
function manager(selector, add, edit) {
    ...
    function addProduct(event) {
        if (event.target.style.backgroundColor == 'var(--primary-color)') {
            ...
            var inputDiscount = document.getElementById("inputPhone");
            inputDiscount.value = "";

            var inputPrice = document.getElementById("inputPass");
            inputPrice.value = "";

            var inputQuantity = document.getElementById("inputAddress");
            inputQuantity.value = "";

            var inputSize = document.getElementById("inputEmail");
            inputSize.value = "";




            var inputCategory = document.getElementById("inputCategory").querySelector("option[name='defaul']");
            inputCategory.selected = true;

            var inputBrand = document.getElementById("inputBrand").querySelector("option[name='defaul'");
            inputBrand.selected = true;

            var inputSupplier = document.getElementById("inputSupplier").querySelector("option[name='defaul']");
        ...
        if (event.target.style.backgroundColor == 'var(--primary-color)') {
            ...
            function Delete() {
                if (confirm("Bạn chắc chắn xóa sản phẩm")) {
                    ...
                    $.ajax({
                        type: "POST",
                        url: "http://localhost:8087/deleteSelectedUser",
                        ...
                        error: function(error) {
                            alert("Xoá không thành công"+error);
                        }
                    });
                    document.getElementById("search ").style.display = 'none';
                    document.querySelector('.cart-detail').style.display = 'none';
